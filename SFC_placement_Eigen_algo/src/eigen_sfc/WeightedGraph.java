...
public class WeightedGraph {
	...
	public DoubleMatrix construireMatriceWithIndirectLink(WeightedGraph G, DoubleMatrix matrix) {
			...
			for (int n = 0; n < G.nbNodes(); n++) {
				...
				if (adgencyConnex[i][n] == null) {
					...
					if (adgencyConnex[n][i] != null)
						adgencyConnex[i][n] = new Cost(0, Dijkstra.dist[n], 0, adgencyConnex[n][i].getPath());
					else
					...
					// System.out.println("Direct link [" + i + " " + n + "] : "
					// + adgencyConnex[i][n].getBandwidth());
	...
	public HashMap<Integer, ArrayList<Server>>shortestPath_and_cost(WeightedGraph G, int src, int dst) {
